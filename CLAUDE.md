...
- 所有 worktree。活躍根唯一：`~/Doc/ai-company-worktrees/`（兩層 `<topic>/<layer>-<module>` 慣例、空殼 rmdir 只認此根）。dirty 偵測靠 `git worktree list` 自報、與根路徑無關，已天然覆蓋所有根
